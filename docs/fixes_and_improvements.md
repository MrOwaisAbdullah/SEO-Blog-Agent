...
## Search Console integration, deeper SEO reporting, and two new review stages

Per explicit request to connect Search Console and have the pipeline actually use it, not just report raw numbers. Added `lib/search_console.py`: a raw-REST client (`google.oauth2.service_account.Credentials` + `webmasters.readonly` scope, not `google-api-python-client`) with `get_page_performance`, `get_page_queries`, `find_striking_distance_queries` (position 5-15 with real impressions), and `get_page_country_device_breakdown`. The Discord bot keeps its own small independent copy of the query helper (`_search_console_query` in `discord_bot/bot.py`) rather than importing `lib/`, consistent with the bot's separate Docker build context.

`/seo_report` (and the equivalent chat tool) now shows site totals (7d/28d), top pages, top queries, top countries (mapped from ISO codes to full names via a ~80-entry `_COUNTRY_NAMES` table), and device split -- not just a one-line "here are your clicks" summary. The weekly digest also includes real Search Console totals instead of only sheet-derived counts.

Two new pipeline stages consume this data directly: `freshness_sweep` (fact-checks time-sensitive claims in old posts against the live web) and `search_performance_review` (finds a published post ranking with real impressions but low CTR/high position and asks whether the title/meta needs a rewrite). Both stamp a "Last Freshness Check"/"Last Performance Check" column that always overwrites, so candidate selection rotates through never-checked-first then least-recently-checked, instead of picking the same "oldest eligible" post forever once one first qualifies.

### `timedelta` NameError crashing `search_performance_review`

Live-reported crash: `name 'timedelta' is not defined`. `scripts/run_stage.py` imported `datetime, timezone` but both new review stages' age math needed `timedelta` too. One-line import fix, verified live.

### Old (pre-pipeline) posts invisible to both review stages

After the `timedelta` fix, `search_performance_review` still reported "No published posts older than 35 days... to check" despite the site having posts from months earlier. Root cause, the same lesson as the duplicate-post fix earlier in this doc but hitting a different code path: candidate age-gating required a sheet-side "Created At" column that only exists for rows created after that column was added -- every pre-existing post had no row at all, so it was permanently invisible to the age filter. Worse, `search_performance_review`'s candidate loop was iterating `generated_posts` sheet rows directly, so even after sourcing age correctly, untracked posts (zero sheet row) never got reached at all.

Fixed by adding `SanityAdapter.list_posts()` (`*[_type == "post"]{title, "slug": slug.current, summary, _createdAt}`) as the universal age/summary source -- Sanity's own `_createdAt` exists on every document regardless of pipeline history, unlike anything sheet-side. `search_performance_review` now iterates this Sanity list directly instead of the sheet (`_select_review_candidate_from_sanity`), using the sheet only as an optional rotation-stamp lookup. Verified live: the stage found and checked "AI Agents, Automations, and Agentic AI - What's Really Different?", a post with zero `generated_posts` row.

## Repurpose redesigned: recommend-only, angle suggestions, image prompts

Per explicit request ("I don't want it auto, it will just recommend me the posts to repurpose, I will decide") plus "use @social-media-writer skill for the repurpose content": `run_repurpose()` now has two modes gated by whether `REPURPOSE_TITLE` is set. Unset: lists every not-yet-repurposed successful post (newest first, capped at 10), each with a one-line repurposing angle suggestion from a new cheap `repurpose_angle_agent`, posted to Discord as a recommendation list only -- nothing gets drafted. Set (via `/repurpose <title>`, the chat tool, or `repurpose_title` workflow input): drafts LinkedIn + Reddit copy for that one specific post via `repurposing_agent`, rewritten to follow the social-media-writer skill's voice (banned words, LinkedIn 700-1000 char sweet spot, scannability rules, "builder not guru" perspective) instead of generic marketing copy. Per follow-up request, `repurposing_agent` also now generates an `image_prompt` field describing a concrete AI-image prompt for the post's angle, saved to the `repurposed_content` worksheet (`_REPURPOSED_CONTENT_HEADERS` extended) and included in the Discord notification.

### Repurpose angle suggestions silently missing for 9/10 candidates

Live-reported gap: the recommendation list showed an angle for only 1 of 10 posts. Root cause: the angle feature's `summaries` dict was built solely from the `generated_posts` sheet's Summary column, so any post without a sheet row (the same untracked-post problem as above) silently got `summaries.get(title, "") == ""` and the code skipped calling `repurpose_angle_agent` entirely -- zero error output, confirmed via the GitHub Actions log showing the agent invoked only once for ten candidates. Fixed by extending `list_posts()`'s query to also return `summary`, sourcing the `summaries` dict from Sanity first and falling back to the sheet only if Sanity has nothing, plus an explicit `print()` log line when neither source has a summary -- specifically so this exact silent-gap class can't recur invisibly again.

### Removed the daily repurpose cron

`pipeline.yml` ran `repurpose` on a `30 9 * * *` daily cron, re-posting the full (largely unchanged) recommendation list to Discord every day -- confirmed live across several days of near-identical "Posts ready to repurpose" messages. This was the *only* way the recommend-only listing ever appeared, since the chat tool and `/repurpose` both require a title. Removed the cron entry entirely; the stage is still fully reachable on demand (ask the Discord chat agent what's worth repurposing, which calls `trigger_stage_tool` with no title, or `gh workflow run -f stage=repurpose`), it just never fires on its own now.

### Repurposing content brought up to the humanizer-main / social-media-writer bar

`repurposing_agent` only had a partial banned-words list and none of the Anti-AI-Pattern Checklist the rest of the pipeline uses. Brought it up to the same bar as `content_generator_agent`/`post_editor_agent`: the full checklist (no em dashes, no copula avoidance, no vague attributions, no rule-of-three padding, etc.), the expanded banned-words list, the explicit "would this feel honest to say to a teammate or mentor" perspective check, a choice of post structures (Problem→Process→Outcome→Lesson, Hook→Setup→Value→Takeaway→CTA, or Situation→Action→Result) instead of one fixed shape, and a final self-check list before returning.

## `edit_post` hardening: fuzzy title matching, real internal links, evaluation, Sanity fallback

A cluster of related `edit_post` bugs, all following from the same root lesson as the duplicate-post fix: the `generated_posts` sheet and a post's live Sanity title are not guaranteed to match.

**Live Sanity patch silently skipped when titles diverge.** `find_post_by_title` did an exact GROQ match against the sheet's `Title` field -- but the Preparation Agent can rephrase/SEO-optimize a title at publish time, so the sheet's title and the live title can legitimately differ (confirmed live: sheet had "OpenAI ChatGPT Update GPT-5.6 Luna", Sanity had "OpenAI ChatGPT Update: GPT-5.6 Luna, Free Unlimited Chats & Price Cuts"). The sheet got updated but the live site never did, three attempts in a row. Fixed with `_find_sanity_post_fuzzy` (normalized substring match, else `difflib` similarity ratio, 0.6 confidence floor) as a fallback when the exact match misses; `list_posts()` now also returns `_id` so this doesn't need a second lookup. Verified live, patch succeeded and the edited sentence confirmed present on the live page.

**Hallucinated internal links.** The content-generation prompt's own worked examples used a fake internal link, `[AI Tips](/blog/ai-tips)`, as filler text in three places -- the model was pattern-matching that literal fake link into real generated posts instead of treating it as a placeholder, confirmed live on a published post. Fixed by replacing all three examples with the correct full-URL format (`fetch_internal_links_tool` already returns full `https://owaisabdullah.dev/blog/...` URLs, never relative paths) and adding explicit rules to both `content_generator_agent` and `content_evaluation_agent`: an internal link must come from an actual tool result this run, never a guessed slug, and should be omitted entirely rather than invented. `post_editor_agent` (no tool access) is told to never add a new internal link at all, since it has no way to verify one is real.

**`edit_post` skipped the pipeline's own quality bar.** `post_editor_agent` was a bare `tools=[]` agent with only a "preserve everything, make the smallest change" prompt -- no banned-words check, no Anti-AI-Pattern Checklist, no self-evaluation, so edited text could read at a lower bar than freshly generated content. It now gets `get_author_context_tool`, `fetch_internal_links_tool`, and `content_evaluation_agent` as a tool (`get_evaluation_feedback`) -- the same pattern `content_generator_agent` already uses to self-evaluate and iterate -- with instructions to apply the same checklist to whatever new text it writes and fix only issues in the edited section (explicitly told to ignore feedback about untouched parts of the post, since this is a scoped edit, not a rewrite). `run_edit_post` now passes Title/Summary/FAQs context so the evaluator has enough to work with. Verified live twice: once via the normal Gemini path, once via a mid-run 429 that fell back to a different model -- both completed and patched correctly.
