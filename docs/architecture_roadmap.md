...
| Search-performance-driven edit loop | Search Console API (section 3) | Medium | Medium | **Implemented, with query-level depth** -- access confirmed live. `search_performance_review` now checks striking-distance queries first (real search terms already getting impressions at position 5-15, naming the exact phrase to work into the content -- the specific, actionable finding), falling back to the blunt low-CTR/poor-position page-level heuristics only when no such query exists. Country/device breakdown attached as reviewer context on any flagged post. |
