...
| Search-performance-driven edit loop | Search Console API (section 3) | Medium | Medium | **Implemented, with query-level depth + real rotation** -- access confirmed live. `search_performance_review` checks striking-distance queries first (real search terms already getting impressions at position 5-15, naming the exact phrase to work into the content), falling back to the blunt low-CTR/poor-position page-level heuristics only when no such query exists. Country/device breakdown attached as reviewer context. Same age-source and rotation fix as freshness sweep above ("Last Performance Check" column, real Sanity `_createdAt`). |
